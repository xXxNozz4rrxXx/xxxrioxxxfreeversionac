...
TriggerEvent("cgb:getLibraryTIGOVERSION",function(cgb) AntiCheat.CGB = cgb end)
...
AntiCheat.Discord.GenerateFooter = function(prefix)
    ...
    if (prefix == nil) then
        return AntiCheat.Render("TigoAntiCheat by PoodFood | 3.2.0 @ {{{time}}}", {
...
AntiCheat.Event.PlayerConnecting = function(playerId, setCallback, deferrals)
    ...
    if (AntiCheat.IsPlayerBypassed(playerId)) then
        if AntiCheat.CGB == nil or AntiCheat.GlobalBanSystem == false then
            deferrals.done()
        else
            AntiCheat.CGB.PlayerJoinEvent(playerId,deferrals)
        end
        return
    end
...
AntiCheat.Player.BanPlayer = function(playerId, reason)
    ...
    if AntiCheat.CGB == nil or AntiCheat.GlobalBanSystem == false then
        ...
        AntiCheat.CGB.BanPlayer(playerId,reason,{day=9999},true,true,"TigoAntiCheat",function(res,banid,global)
            if res == true then
                if global then
